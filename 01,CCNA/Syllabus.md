# Networking 

##  Fundamentals
- [ ] OSI Model (L1–L7)
- [ ] TCP/IP Model
- [ ] TCP vs UDP
- [ ] Encapsulation / Decapsulation
- [ ] MTU + Fragmentation (basic)

---

##  IP Addressing & Subnetting (Must)
- [ ] IPv4 structure
- [ ] Private vs Public IP ranges
- [ ] Loopback (127.0.0.1)
- [ ] APIPA (169.254.x.x)
- [ ] CIDR notation (/24, /16, etc.)
- [ ] Network ID vs Broadcast vs Host range
- [ ] Subnetting basics (/24, /25, /26, /27)
- [ ] Default Gateway concept

---

##  Switching Concepts (Layer 2)
- [ ] MAC Address basics
- [ ] ARP working (Request/Reply)
- [ ] ARP Table meaning
- [ ] Switch vs Hub vs Router
- [ ] VLAN basics
- [ ] Trunk vs Access ports (basic)

---

## Routing Concepts (Layer 3)
- [ ] Routing basics
- [ ] Route table basics
- [ ] Static vs Dynamic routing (high level)
- [ ] Traceroute working
- [ ] ICMP basics (Echo, TTL)

---

##  NAT & Network Boundaries
- [ ] NAT concept
- [ ] SNAT vs DNAT
- [ ] PAT (NAT Overload)
- [ ] Port Forwarding
- [ ] DMZ concept

---

##  Firewalls & Filtering
- [ ] Stateful vs Stateless firewall
- [ ] Inbound vs Outbound rules
- [ ] Egress filtering
- [ ] ACL basics
- [ ] Open vs Closed vs Filtered ports

---

##  Ports & Services (Very Important)
- [ ] What is a port?
- [ ] What is a service?
- [ ] Well-known vs Ephemeral ports
- [ ] Common TCP ports
- [ ] Common UDP ports

---

##  Core Protocols (Must Know)

### DNS
- [ ] DNS working
- [ ] A / AAAA / CNAME / MX / NS / TXT records
- [ ] Forward DNS vs Reverse DNS
- [ ] Zone transfer concept
- [ ] Subdomain enumeration concept

### HTTP/HTTPS
- [ ] HTTP request/response
- [ ] Methods: GET, POST, PUT, DELETE
- [ ] Headers + Cookies basics
- [ ] Sessions vs Tokens (basic)
- [ ] HTTPS + TLS basics

### DHCP
- [ ] DHCP working
- [ ] DORA process
- [ ] DHCP scope concept

---

## Windows / Internal Network Protocols (VAPT Gold)
- [ ] SMB (445)
- [ ] NetBIOS (137–139)
- [ ] LDAP (389/636)
- [ ] Kerberos (88)
- [ ] RDP (3389)
- [ ] WinRM (5985/5986)

---

## Remote Access Protocols
- [ ] SSH (22)
- [ ] FTP (21) + insecure
- [ ] Telnet (23) + insecure
- [ ] VPN basics (site-to-site vs remote access)
- [ ] Split tunnel vs Full tunnel

---

## Scanning & Enumeration (Practical VAPT)
- [ ] Host discovery concepts
- [ ] Port scanning basics
- [ ] SYN scan vs TCP connect scan
- [ ] UDP scan basics (why slow)
- [ ] Banner grabbing
- [ ] Service fingerprinting
- [ ] Why scans give false positives

---

##  Packet & Traffic Analysis
- [ ] PCAP concept
- [ ] Wireshark basics
- [ ] TCP 3-way handshake
- [ ] TCP flags (SYN, ACK, FIN, RST, PSH)
- [ ] Retransmissions + timeouts (basic)
- [ ] Identify DNS + HTTP in Wireshark

---

##  Bonus (Good for Interviews + Real Work)
- [ ] Proxy vs Reverse Proxy
- [ ] Load balancer basics
- [ ] WAF basics
- [ ] CDN basics
- [ ] Zero Trust concept
- [ ] Network segmentation importance
