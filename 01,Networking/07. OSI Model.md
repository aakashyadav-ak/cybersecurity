...
It starts a communication session between two applications.
...
The session layer keeps the communication active.

##### It manages things like:
1. Dialogue control (who sends data)
	Example:
		```
		Client: Send request
		...
		Client: Next request
		```
	The session layer coordinates the order of communication, not the physical transmission mode.


2. **Synchronization:** Adds checkpoints during long data transfers.
		If communication fails, it resumes from the last checkpoint instead of restarting completely.


3. Session recovery

	**Example:**
	If a large file transfer stops halfway, the session layer can allow resume from checkpoint instead of restarting.


#### 3. Session Termination
When communication ends, the session layer closes the session cleanly.
